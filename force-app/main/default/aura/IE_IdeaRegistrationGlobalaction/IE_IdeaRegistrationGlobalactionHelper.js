...
({
  submitIdea: function(cmp, ideaTitleModal, ideaBodyModal, categoryModal, isPost, recordId, point, isAnonymous) {
    // サーバ処理呼出
  ...
  requiredErrorCheck: function(cmp, isPost) {
    ...
    let isAnonymous = cmp.get('v.isAnonymous');
    let errorItemList = [];
    let errorItem = '';
    ...
    if (errorItem == '' || errorItem == null) {
      this.submitIdea(cmp, ideaTitleModal, ideaBodyModal, categoryModal, isPost, recordId, point, isAnonymous);
